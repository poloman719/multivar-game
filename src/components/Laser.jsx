...
const Laser = ({start, end}) => {
  ...
  const [laserTexture, startLaserTexture] = useAnimation("laser-animation/laser sprite0", "png", 92, 1);  
  const positionVec = new Vector3(start[0], start[1], start[2]).addScaledVector(distanceVec, 0.5)
  ...
  return (
    <>
      <mesh position={positionVec} scale={distanceVec.length() * 1.7543} rotation={rotation} ref={plane}>
        <planeGeometry />
        <meshBasicMaterial transparent ref={plane} map={laserTexture} side={DoubleSide}/>
      </mesh>
